...
XS_CLASS_BEGIN_WITH_ANCESTOR_WITH_COPY( GRAPHIC_SHADER_EFFECT, GRAPHIC_SHADER_EFFECT_RESOURCE_ANCESTOR )
    ...
    inline void SetMaterialCollection( GRAPHIC_MATERIAL_COLLECTION::PTR collection ) { MaterialCollection = collection; }
    inline void SetMaterial( GRAPHIC_MATERIAL::PTR material, const char * name = DefaultMaterialName ) { MaterialCollection->SetMaterialForName( material, name ); }
    inline GRAPHIC_MATERIAL::PTR GetMaterial( const char * name = DefaultMaterialName ) { return MaterialCollection->GetMaterialForName( name ); }
