...
bool WICFileIO::SaveToJPEG(std::string const& filename, Texture2* texture,
    float imageQuality)
{
...
bool WICFileIO::SaveTo(std::string const& filename, Texture2* texture,
    float imageQuality)
{
    ...
    {
        hr = wicFactory->CreateEncoder(GUID_ContainerFormatJpeg, nullptr,
            &wicEncoder);
    }
    if (FAILED(hr))
    {
