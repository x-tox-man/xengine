...
void ETManifoldMesh::Print(std::ostream& output) const
{
    if (output.bad())
    {
...
void ETManifoldMesh::AssignUniqueIndices(std::map<Edge*, int>& eIndex,
...
void ETManifoldMesh::PrintEdgesTriangles(std::ostream& output,
    std::map<Edge*, int>& eIndex, std::map<Triangle*, int>& tIndex) const
{
    output << "edge quantity = " << mEMap.size() << std::endl;
    for (auto const& element : mEMap)
    {
        Edge const& edge = *element.second;
    ...
    output << "triangle quantity = " << mTMap.size() << std::endl;
    for (auto const& element : mTMap)
    {
        Triangle const& tri = *element.second;
...
ETManifoldMesh::Triangle* ETManifoldMesh::CreateTriangle(int v0, int v1, int v2)
{
