...
void NETWORK_SERVER::ProcessIncommingMessages() {
    ...
    for (int i = 0; i < IncommingMessageQueue.size(); i++ ) {
        ...
        if ( command != NULL ) {
            ...
            int type = ((APPLICATION_COMMAND*) event->GetCommand())->GetCommandType();
            
            if ( type == (int) APPLICATION_NETWORK_COMMAND_ClientConnected ) {
            ...
            else if ( type == (int) APPLICATION_NETWORK_COMMAND_ClientQuit ) {
                
                event->GetCommand();
                StateMachine.DispatchEvent( CLIENT_DISCONNECTED_EVENT() );
            }
        }
