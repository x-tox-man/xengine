...
private:
    ...
    // To update the VCVertex information during collapses, we need to modify
    // const data that is accessed from mMesh.  A const_cast is perfomed by
    // this function.
    inline VCVertex* DoCast(VETManifoldMesh::Vertex const* vertex);
    ...
    mMesh(VCVertex::Create)
{
    ...
    {
        auto const* edge = eelement.second;
        if (!edge->T[1])
        {
            for (int i = 0; i < 2; ++i)
            {
                auto velement = vmap.find(edge->V[i]);
                VCVertex* vertex = DoCast(velement->second);
...
template <typename Real> inline
typename VertexCollapseMesh<Real>::VCVertex*
VertexCollapseMesh<Real>::DoCast(VETManifoldMesh::Vertex const* vertex)
{
    return static_cast<VCVertex*>(const_cast<VETManifoldMesh::Vertex*>(vertex));
}

template <typename Real>
int VertexCollapseMesh<Real>::TriangulateLink(VCVertex* vertex,
    ...
    for (size_t i0 = numVertices - 1, i1 = 0; i1 < numVertices; i0 = i1++)
    {
        ...
        if (edge->T[0] && !edge->T[1])
        {
            for (int k = 0; k < 2; ++k)
            {
                ...
                VCVertex* vertex = static_cast<VCVertex*>(
                    const_cast<VETManifoldMesh::Vertex*>(velement->second));
                vertex->isBoundary = true;
            }
        }
    }
...
template <typename Real>
VETManifoldMesh::Vertex* VertexCollapseMesh<Real>::VCVertex::Create(int v)
{
    return new VCVertex(v);
}
