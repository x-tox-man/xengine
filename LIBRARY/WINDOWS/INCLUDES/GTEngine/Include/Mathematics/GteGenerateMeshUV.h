...
#if defined(GTE_COMPUTE_MODEL_ALLOW_GPGPU)
        UVComputeModel(unsigned int inNumThreads, GraphicsEngine* inEngine,
            ProgramFactory* inFactory, std::function<void(unsigned int)> const* inProgress);
#endif
...
template <typename Real>
GenerateMeshUV<Real>::UVComputeModel::UVComputeModel(unsigned int inNumThreads, GraphicsEngine* inEngine,
    ProgramFactory* inFactory, std::function<void(unsigned int)> const* inProgress)
    :
