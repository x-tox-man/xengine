...
public:
    ...
    void GetComponents(std::vector<std::vector<Triangle const*>>& components) const;
...
protected:
    ...
    // For debugging.
    void AssignUniqueIndices(std::map<Edge*, int>& eIndex, std::map<Triangle*, int>& tIndex) const;
    void PrintEdgesTriangles(std::ostream& output, std::map<Edge*, int>& eIndex,
